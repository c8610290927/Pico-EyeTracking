            ...
            public static class SRanipal_Eye
            {
                ...
                public static bool GetGazeRay(GazeIndex gazeIndex, out Vector3 origin, out Vector3 direction)
                {
                    ...
                    if (SRanipal_Eye_Framework.Status != SRanipal_Eye_Framework.FrameworkStatus.WORKING)
                    {
                        ...
                        origin = GameObject.Find("Pvr_UnitySDK").transform.GetChild(1).GetComponent<Camera>().transform.position;
                        valid = true;
                    }
                    else{
                        UpdateData();
                ...
                public static bool GetGazeRay(GazeIndex gazeIndex, out Ray ray)
                {
                    ...
                    {
                        //rigin = Camera.main.transform.position;
                        origin = GameObject.Find("Pvr_UnitySDK").transform.GetChild(1).GetComponent<Camera>().transform.position;
                        valid = true;
                    }
                ...
                public static bool Focus(GazeIndex index, out Ray ray, out FocusInfo focusInfo, float radius, float maxDistance)
                {
                    ...
                    if (valid)
                    {
                        ...
                        Ray rayGlobal = new Ray(GameObject.Find("Pvr_UnitySDK").transform.GetChild(1).GetComponent<Camera>().transform.position, GameObject.Find("Pvr_UnitySDK").transform.GetChild(1).GetComponent<Camera>().transform.TransformDirection(ray.direction));
                        RaycastHit hit;
